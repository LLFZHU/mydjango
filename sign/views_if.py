...
def add_event(request):
    ...
    if result:
        return JsonResponse({'status':1023, 'message':'event name already exists'})
    if status == '':
        status = 1
    try:
        ...
        return JsonResponse({'status':1024,'message':error})
...
def add_guest(request):
    ...
    try:
        ...
        return JsonResponse({'status':10026, 'message':'the event guest phone numbere repeat'})
